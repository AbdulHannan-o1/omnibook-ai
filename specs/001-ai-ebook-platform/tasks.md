...
- [x] T001 Create root project directory structure: `/book`, `/frontend`, `/backend`
- [x] T002 Create root-level `package.json` (optional, for shared commands)
- [x] T003 Create root-level `README.md` explaining the monorepo structure
...
### Implementation Tasks:
- [x] T004 [US-Docusaurus] Initialize Docusaurus project in `book/`
- [x] T005 [US-Docusaurus] Configure MDX support in `book/docusaurus.config.js`
- [x] T006 [US-Docusaurus] Add theme toggles, sidebar, footer, navbar, and layouts in `book/docusaurus.config.js` and theme files
- [x] T007 [P] [US-Docusaurus] Create placeholder `chapter-1-introduction.mdx` in `book/docs/`
- [x] T008 [P] [US-Docusaurus] Create placeholder `chapter-2-ai-agents.mdx` in `book/docs/`
- [x] T009 [P] [US-Docusaurus] Create placeholder `chapter-3-rag-systems.mdx` in `book/docs/`
- [x] T010 [US-Docusaurus] Add “What’s on this page” component using a custom MDX wrapper in `book/src/components/`
- [x] T011 [US-Docusaurus] Configure search (local or algolia stub) in `book/docusaurus.config.js`
- [x] T012 [US-Docusaurus] Set book title to **OmniBook-AI** in `book/docusaurus.config.js`
...
### Implementation Tasks:
- [x] T013 [US-Frontend] Initialize Vite + React + TypeScript project in `frontend/`
- [x] T014 [US-Frontend] Build main routes (`/`, `/book`, `/chat`) in `frontend/src/App.tsx` and `frontend/src/router.tsx`
- [x] T015 [P] [US-Frontend] Integrate Header / Footer components in `frontend/src/components/`
- [x] T016 [P] [US-Frontend] Integrate Table of Content preview panel component in `frontend/src/components/`
- [x] T017 [P] [US-Frontend] Integrate Chapter cards component in `frontend/src/components/`
- [x] T018 [US-Frontend] Implement API client to call FastAPI backend (stub) in `frontend/src/services/api.ts`
- [x] T019 [US-Frontend] Build simple chatbot UI (input box, messages, streaming optional) in `frontend/src/pages/Chat.tsx`
...
### Implementation Tasks:
- [x] T020 [US-Backend] Initialize FastAPI project in `backend/`
- [x] T021 [US-Backend] Create proper folder structure (`backend/routers/`, `backend/services/`, `backend/schemas/`)
- [x] T022 [P] [US-Backend] Implement `POST /query` API route (stub) in `backend/routers/rag.py`
- [x] T023 [P] [US-Backend] Implement `GET /chapters` API route (metadata list, stub) in `backend/routers/chapters.py`
- [x] T024 [P] [US-Backend] Implement `GET /chapters/{id}` API route (return chapter content, stub) in `backend/routers/chapters.py`
- [x] T025 [US-Backend] Add CORS support for Vite + Docusaurus domains in `backend/main.py`
...
### Implementation Tasks:
- [x] T026 [US-Chapters] Generate first 3 full chapters using agent pipeline
- [x] T027 [US-Chapters] Convert generated chapters to MDX and place in `book/docs/`
- [x] T028 [US-Chapters] Add chapters to `book/sidebars.json`
- [x] T029 [US-Chapters] Index chapters into RAG storage (JSON, SQLite, ChromaDB, etc.)
- [x] T030 [US-Chapters] Test FastAPI `/chapters` endpoint to return expected data
- [x] T031 [US-Chapters] Test FastAPI `/query` endpoint to return expected data
...
### Implementation Tasks:
- [x] T032 [US-Integration] Connect Vite frontend `/chat` page to FastAPI `/query`
- [x] T033 [US-Integration] Link Docusaurus pages inside the Vite navigation menu
- [x] T034 [US-Integration] Ensure unified branding and UI theme across both systems
...
### Implementation Tasks:
- [x] T035 [P] [US-Quality] Add `.env` templates for frontend (`frontend/.env.example`) + backend (`backend/.env.example`)
- [x] T036 [US-Quality] Add initial tests for FastAPI routes in `backend/tests/`
- [x] T037 [US-Quality] Add GitHub repo actions placeholders (optional) in `.github/workflows/`
- [x] T038 [US-Quality] Prepare containerization instructions (Dockerfile stubs only) in root and `/backend` and `/frontend`
